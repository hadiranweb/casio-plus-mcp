...
describe('bank store', () => {
  it('stores summaries and returns them ordered by month', () => {
    ...
    store.upsert(s('7001', 'General Operations', '2026-04', 2450075, 2015025));
    store.upsert(s('7002', 'Vantage', '2026-04', 2100000, 2480000));
  ...
  it('re-uploading the same account+month updates rather than duplicates', () => {
    ...
    expect(store.all()[0].creditsCents).toBe(2450075);
    expect(store.all()[0].netCents).toBe(2450075 - 2015025);
  });
});
