...
function greeting(): string {
  ...
  if (hour < 5) return t('home.greet.late');
  if (hour < 12) return t('home.greet.morning');
  if (hour < 18) return t('home.greet.afternoon');
...
export default async function HomePage() {
  ...
  // Runs store agent ids; show the roster display name (falls back to the id).
  const agentNames = new Map(db.agents.all().map((a) => [a.id, a.name]));
...
@media (prefers-reduced-motion: reduce) { .os-ticker-track { animation: none; } }`,
      ...
      <PageHeader
        ...
        title={`${greeting()}، ${t('home.greet.name')}`}
        caret
        right={<Kbd>⌘K</Kbd>}
      />
      ...
      <section className="mb-[18px] grid grid-cols-4 gap-3 max-[1100px]:grid-cols-2">
        <StatTile
          href="/integrations"
          label={t('home.pulse.systems')}
          value={num(connected)}
          unit={t('home.pulse.systems.unit', { total: num(connections.length) })}
          foot={<ConnectorBars connections={connections} />}
        />
        <StatTile
          href="/agents"
          label={t('home.pulse.agents')}
          value={num(activeAgents)}
          unit={t('home.pulse.agents.unit', { total: num(agents.length) })}
          foot={<Spark data={agentsSpark} />}
        />
        <StatTile
          href="/comms"
          label={t('home.pulse.comms')}
          value={num(inbound)}
          unit={t('home.pulse.comms.unit')}
          foot={<Spark data={commsSpark} />}
        />
        <StatTile
          href="/brain"
          label={t('home.pulse.brain')}
          value={health == null ? '—' : num(health)}
          unit={t('home.pulse.brain.unit', { status: overview.doctor.connected ? t(`status.${overview.doctor.status}`) : t('home.pulse.brain.offline') })}
      ...
      <section className="mb-[22px] relative overflow-hidden rounded-md-t border border-os-border bg-os-surface">
        <div className="absolute inset-y-0 left-0 z-[2] flex items-center gap-[7px] border-r border-os-border bg-os-surface px-3.5 font-mono text-[9.5px] uppercase tracking-[0.18em] text-os-dim">
          <Dot state="connected" pulse /> {t('home.ticker.live')}
        </div>
            ...
            <div key={copy} className="flex shrink-0 gap-[26px] pr-[26px]">
              {ticker.map((r) => (
                <span key={copy + r.id} className="inline-flex items-center gap-2 whitespace-nowrap font-mono text-[11px]">
                  ...
                  <span className="text-os-muted">{agentNames.get(r.agentId) ?? r.agentId}</span>
      ...
      <section className="mb-[22px]">
        <SectionHead label={t('home.connections.title')} count={`${num(connected)}/${num(connections.length)}`} link={t('home.connections.open')} href="/integrations" />
      ...
      <section className="mb-[22px]">
        <SectionHead label={t('home.social.title')} count={t('home.social.count')} link={t('home.social.open')} href="/social" />
      ...
      <div className="grid grid-cols-[1.05fr_0.95fr] items-start gap-6 max-[1100px]:grid-cols-1">
        ...
        <section className="min-w-0">
          <SectionHead label={t('home.agents.title')} count={t('home.agents.count', { count: num(activeAgents) })} link={t('home.agents.open')} href="/agents" />
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {a.tier === 'lead' && <Badge>{t('home.agents.lead')}</Badge>}
                    </div>
                    <div className="mt-0.5 truncate font-mono text-[10.5px] text-os-dim">
                      {departments.get(a.departmentId) ?? '—'} ·{' '}
                      {last ? t('home.agents.lastRun', { result: t(last.ok ? 'home.agents.result.ok' : 'home.agents.result.failed'), time: relativeTime(last.finishedAt) }) : t('home.agents.neverRun')}
                    </div>
                  </div>
                  <span
                  ...
                  >
                    {a.status === 'active' ? t('home.agents.run') : t('home.agents.noCreds')}
                  </span>
                </Link>
              );
            })}
        ...
        <section className="flex min-w-0 flex-col gap-[22px]">
          <div>
            <SectionHead label={t('home.runs.title')} count={num(recentRuns.length)} />
            <ul className="flex flex-col gap-1.5">
              {recentRuns.slice(0, 6).map((r) => (
                ...
                >
                  ...
                  <span className="shrink-0 text-os-muted">{agentNames.get(r.agentId) ?? r.agentId}</span>
          ...
          {nowItems.length > 0 && (
            <div>
              <SectionHead label={t('home.now.title', { quarter: nowQuarter?.quarter ?? t('home.now.fallback') })} link={t('home.roadmap.open')} href="/roadmap" />
              <ul className="flex flex-col gap-1.5">
                {nowItems.map((it) => (
                  <li
                    key={it.id}
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <Label>{t('home.brain.label')}</Label>
              <div className="mt-2 text-[13px] font-semibold">{t('home.brain.headline')}</div>
              <div className="mt-1 font-mono text-[10.5px] leading-relaxed text-os-dim">
                {t('home.brain.detail', {
                  pages: num(overview.store.totalFiles),
                  health: health == null ? '—' : num(health),
                  state: t(overview.doctor.connected ? 'home.brain.verified' : 'home.brain.degraded'),
                })}
              </div>
            </div>
